      ...
      <div class="flex items-center justify-between h-14">
        ...
        <div class="flex items-center gap-1">
          <router-link to="/" class="nav-link" :class="{ active: $route.name === 'market' }">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM14 14h7v7h-7z"/></svg>
            市场
          </router-link>

          <template v-if="isLoggedIn">
            <router-link v-if="isCustomer || isDeveloper || isAdmin" to="/dashboard" class="nav-link" :class="{ active: $route.name === 'dashboard' }">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
              控制台
            </router-link>
            <router-link v-if="isCustomer || isAdmin" to="/chat" class="nav-link" :class="{ active: $route.name === 'chat' }">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
              对话
            </router-link>
            <router-link v-if="isDeveloper || isAdmin" to="/developer" class="nav-link" :class="{ active: $route.name === 'developer' }">
            ...
            <div class="relative" ref="menuRef">
              ...
              <div v-if="showMenu" class="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
                <div class="px-4 py-2 text-xs text-gray-500 border-b">
                  {{ { customer: '客户', developer: '开发者', admin: '管理员' }[role] || role }}
                </div>
